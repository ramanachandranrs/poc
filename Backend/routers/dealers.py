...
router = APIRouter(
    ...
    tags=["dealers"],
    dependencies=[Depends(check_role([models.UserRole.MANAGER]))]
)
...
def create_dealer(dealer: DealerCreate, db: Session = Depends(get_db), current_user: models.AppUser = Depends(get_current_active_user)):
    role_str = str(current_user.role).lower()
    if ("manager" in role_str or "regional_distributor" in role_str) and dealer.zone != current_user.zone:
...
def update_dealer(dealer_id: str, dealer: DealerUpdate, db: Session = Depends(get_db), current_user: models.AppUser = Depends(get_current_active_user)):
    ...
    role_str = str(current_user.role).lower()
    if ("manager" in role_str or "regional_distributor" in role_str) and db_dealer.zone != current_user.zone:
